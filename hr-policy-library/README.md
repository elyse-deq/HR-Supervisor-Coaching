...
cd HR-Supervisor-Coaching/hr-policy-library
python -m venv .venv
...
CI for both servers runs from `.github/workflows/tests.yml` at the repository root.

## Companion server

`hr-policy-library` answers what policy says and when HR must be involved. Its companion, [`learning-coaching-resources`](../learning-coaching-resources), serves the approved coaching guides, training, manager resources, and documentation templates that help a supervisor hold the conversation. Escalation decisions stay here, so there is one source of truth for when HR is required.
